...
export const GlobalAlertProvider = ({ children }) => {
    ...
    const showAlert = useCallback((message, type = 'info', duration = 5000) => {
        ...
        setTimeoutId(id);
    }, [timeoutId]); // Depend on timeoutId to ensure correct cleanup of previous timeout
    ...
    }, [timeoutId]); // Depend on timeoutId to clear it if component unmounts

    // Value provided by the context - memoized for performance
    ...
    return (
        <GlobalAlertContext.Provider value={alertContextValue}>
            {children}
            {/* Optional: Render a global alert component here (e.g., a Toast or Snackbar) */}
            {alert && (
                <div
