...
// CORRECTED: Switched from react-icons to the project-standard lucide-react library.
import { Plus, Filter, Loader2, ServerCrash } from 'lucide-react';

// --- SERVICE AND CONTEXT IMPORTS ---
// NOTE: Please ensure these paths are correct for your project's file structure.
// The build error suggests they could not be found at these locations.
import { useAuth } from '../../context/AuthContext.jsx';
import { getAllRequests } from '../../services/requestService.js';

// --- MOCK COMPONENTS TO PREVENT ERRORS ---
// I've included functional placeholders for your custom components.
// You can replace these with your actual component imports once the pathing is confirmed.
        ...
        <div className="flex justify-center items-center gap-2 p-4">
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                <button
                    ...
                    className={`h-10 w-10 rounded-md ${currentPage === page ? 'bg-[#219377] text-white' : 'bg-gray-200 text-gray-700'}`}
                >
                    {page}
                </button>
            ))}
...
const MyRequestsPage = () => {
    ...
    const filteredRequests = useMemo(() => {
        return requests.filter(req => {
            const statusMatch = filters.status === 'all' || req.status.toLowerCase() === filters.status;
            const categoryMatch = filters.category === 'all' || req.category.toLowerCase() === filters.category;
    ...
    const uniqueCategories = useMemo(() => 
        [...new Set(requests.map(req => req.category.toLowerCase()))]
    , [requests]);
        ...
        <div className="p-4 md:p-6 bg-gray-50 min-h-full">
            <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">Track the status of all your submitted issues.</p>
                </div>
                <button
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-200">
                {loading ? (
                    ...
                    <p className="text-center py-10 text-gray-500">No requests match your filters. Try adjusting them or submitting a new request.</p>
                ) : (
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {paginatedRequests.map(req => (
                                    <tr key={req._id} className="hover:bg-gray-50 cursor-pointer" onClick={() => navigate(`/tenant/requests/${req._id}`)}>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-emerald-700">{req.title}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{req.property?.name || 'N/A'}</td>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{new Date(req.updatedAt).toLocaleDateString()}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
