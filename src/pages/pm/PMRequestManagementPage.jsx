...
import { getAllRequests, createRequest, updateRequest, assignRequest } from "../../services/requestService";
import { getAllProperties } from "../../services/propertyService";
import { getAllVendors } from "../../services/vendorService";
import { getAllUsers } from "../../services/userService";
import { PlusCircle, Search, UserPlus, CheckCircle, RefreshCcw, Archive, Eye } from 'lucide-react';
...
function PMRequestManagementPage() {
  // Data state
  const [requests, setRequests] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [properties, setProperties] = useState([]); // <-- FIX: Add property state
  ...
  const validateAddRequestForm = () => {
    ...
    if (!addRequestForm.description.trim()) errors.description = "Description is required.";
    if (!addRequestForm.category.trim()) errors.category = "Category is required.";
    if (!addRequestForm.propertyId.trim()) errors.propertyId = "Property is required.";
  ...
  const handleAddRequestSubmit = async (e) => {
    ...
    try {
      ...
      setAddRequestError("Failed to create request: " + (err.response?.data?.message || err.message));
  ...
  // Assignment modal handlers etc - as before
  ...
  const handleUpdateStatus = async (requestId, newStatus) => {
    ...
    try {
      ...
      showMessage(`Request marked as ${newStatus.replace(/_/g, " ")}!`, "success");
  ...
  return (
    <div className="p-4 md:p-8 bg-gray-50 min-h-full">
      <h1 className="text-3xl font-extrabold text-gray-900 mb-6 border-b pb-2">Maintenance Requests</h1>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6 p-4 bg-white rounded-lg shadow-sm border border-gray-100">
        ...
        >
          <PlusCircle className="w-5 h-5" /> <span>Add New Request</span>
        </Button>
        <div className="flex items-center gap-3">
          ...
          <form onSubmit={(e) => { e.preventDefault(); fetchRequests(); }} className="flex items-center gap-2">
            <label htmlFor="filterSearch" className="sr-only">Search Requests</label>
            <input
              type="text"
              id="filterSearch"
                  ...
                  <tr key={req._id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center space-x-2">
                          <Link to={`/pm/requests/${req._id}`} className="text-indigo-600 hover:text-indigo-900" title="View Details"><Eye className="w-5 h-5" /></Link>
                          {req.status === 'new' && (
                            <Button onClick={() => handleOpenAssignModal(req._id)} className="text-purple-600 hover:text-purple-900 p-1 rounded-md" title="Assign">
                              <UserPlus className="w-5 h-5" />
                            </Button>
                          )}
                          {req.status === 'assigned' || req.status === 'in_progress' && (
                            <Button onClick={() => handleUpdateStatus(req._id, 'completed')} className="text-green-600 hover:text-green-900 p-1 rounded-md" title="Mark Completed">
                              <CheckCircle className="w-5 h-5" />
                            </Button>
                          )}
                          {req.status === 'completed' && (
                            <Button onClick={() => handleUpdateStatus(req._id, 'verified')} className="text-teal-600 hover:text-teal-900 p-1 rounded-md" title="Mark Verified">
                              <CheckCircle className="w-5 h-5" />
                            </Button>
                          )}
                          {(req.status === 'completed' || req.status === 'verified') && (
                            <Button onClick={() => handleUpdateStatus(req._id, 'reopened')} className="text-orange-600 hover:text-orange-900 p-1 rounded-md" title="Reopen Request">
                              <RefreshCcw className="w-5 h-5" />
                            </Button>
                          )}
                          {(req.status !== 'archived') && (
                            <Button onClick={() => handleUpdateStatus(req._id, 'archived')} className="text-gray-600 hover:text-gray-900 p-1 rounded-md" title="Archive Request">
      ...
      >
        <form onSubmit={handleAddRequestSubmit} className="p-4 space-y-4">
          ...
          <div>
            <label htmlFor="addRequestTitle" className="block text-sm font-medium text-gray-700">Title</label>
            <input
              type="text"
              id="addRequestTitle"
              name="title"
              ...
              className={`mt-1 block w-full border ${addRequestFormErrors.title ? 'border-red-500' : 'border-gray-300'} rounded-md shadow-sm p-2`}
              required
          ...
          <div>
            <label htmlFor="addRequestDescription" className="block text-sm font-medium text-gray-700">Description</label>
            <textarea
              ...
              onChange={handleAddRequestFormChange}
              rows="4"
              className={`mt-1 block w-full border ${addRequestFormErrors.description ? 'border-red-500' : 'border-gray-300'} rounded-md shadow-sm p-2`}
              required
              disabled={addRequestLoading}
            />
          ...
          <div>
            <label htmlFor="addRequestCategory" className="block text-sm font-medium text-gray-700">Category</label>
            <select
              id="addRequestCategory"
              name="category"
              value={addRequestForm.category}
              onChange={handleAddRequestFormChange}
              className={`mt-1 block w-full border ${addRequestFormErrors.category ? 'border-red-500' : 'border-gray-300'} rounded-md shadow-sm p-2`}
              required
              disabled={addRequestLoading}
            >
          ...
          <div>
            <label htmlFor="addRequestPriority" className="block text-sm font-medium text-gray-700">Priority</label>
            <select
          ...
          <div>
            <label htmlFor="addRequestProperty" className="block text-sm font-medium text-gray-700">Property</label>
            <select
              id="addRequestProperty"
              name="propertyId"
              value={addRequestForm.propertyId}
              onChange={handleAddRequestFormChange}
              className={`mt-1 block w-full border ${addRequestFormErrors.propertyId ? 'border-red-500' : 'border-gray-300'} rounded-md shadow-sm p-2`}
              required
              disabled={addRequestLoading}
            >
          ...
          <div>
            <label htmlFor="addRequestUnit" className="block text-sm font-medium text-gray-700">Unit (optional)</label>
            <select
              id="addRequestUnit"
              name="unitId"
          ...
          <div className="flex justify-end space-x-3 mt-6">
            <Button type="button" onClick={() => setShowAddRequestModal(false)} className="bg-gray-300 hover:bg-gray-400 text-gray-800 py-2 px-4 rounded-lg">Cancel</Button>
            <Button type="submit" className="bg-#219377 hover:bg-[#ffbd59] text-white py-2 px-4 rounded-lg" loading={addRequestLoading} disabled={addRequestLoading}>Add Request</Button>
          </div>
        </form>
      </Modal>
          ...
          <div>
            ...
            >
              ...
              {assignedToModel === 'Vendor' ? (
                vendors.map(v => <option key={v._id} value={v._id}>{v.name} ({v.services?.join(', ')})</option>)
              ) : (
                internalUsers.map(u => <option key={u._id} value={u._id}>{u.name || u.email} ({u.role})</option>)
              )}
            </select>
          </div>
          <div className="flex justify-end space-x-3 mt-6">
